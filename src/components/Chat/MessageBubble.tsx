  ...
  return (
    ...
    >
      ...
      <div className={`max-w-[75%] flex flex-col ${isAssistant ? 'items-start' : 'items-end'}`}>
        <div
          className={`rounded-2xl px-4 py-2.5 text-sm leading-6 whitespace-pre-wrap break-words ${
            isAssistant
        ...
        >
          {isResultCard ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.16em] text-[#FFB8DF]">
                <Sparkles size={14} />
                <span>Nano Banana</span>
                {message.meta?.operation && (
                  <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] tracking-normal normal-case text-surface-300">
                ...
                {message.meta?.mode && (
                  <span className="rounded-full border border-[#FFB8DF]/25 bg-[#FFB8DF]/10 px-2 py-0.5 text-[10px] tracking-normal normal-case text-[#FFB8DF]">
              ...
              {imageAttachment.prompt && (
                <div className="rounded-xl bg-surface-900/70 border border-white/6 px-3 py-2">
                  <p className="text-[10px] font-medium uppercase tracking-[0.16em] text-surface-500">Prompt</p>
                  <p className="mt-1 text-sm leading-6 text-surface-200">{imageAttachment.prompt}</p>
                </div>
              )}
              ...
              <div className="flex items-center justify-between gap-3 text-[11px] text-surface-400">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-surface-600/60 px-3 py-1.5 text-[12px] font-medium text-surface-200 transition-colors hover:border-[#FFB8DF]/40 hover:text-[#FFB8DF]"
                >
                  <Download size={12} />
                  下载
                </a>
              ...
              {message.content && (
                <p className="text-sm leading-6 text-surface-300">{message.content}</p>
              )}
            </div>
          ) : (
            <div className="space-y-3">
              {imageAttachment && (
                <div className="space-y-2">
                  {!isAssistant && (
                    <p className="text-[10px] font-medium uppercase tracking-[0.16em] text-white/70">参考图</p>
                  )}
                  <img
                    src={imageAttachment.url}
        ...
        <span className="mt-1 px-1 text-[10px] text-surface-500">
